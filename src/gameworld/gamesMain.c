...
void gamesMain_dispatchFrame()
{
    ...
    if( confirmingQuit )
    {
        ...
        if( justA )
        {
            ...
            if( confirmSelection == 1 )
            {
                // md_stopTone() alone only clears the one-shot voice pool
                // (gbPlayTick()/OK()/Cancel()) - the tracker engine's own
                // per-channel state (gbTrackIsPlaying[]/gbPatternIsPlaying[]/
                // gbNotePlaying[]/gbActiveVoice[]) isn't reset by it at all,
                // so a game quit mid-track (e.g. 101Starships' own
                // continuous background music) would otherwise leave that
                // state dangling - harmless once a fresh game's own
                // gbBegin() resets it, but real, audible silence is the
                // whole point of quitting to the menu, not "silent until
                // the next launch happens to reset it." gbStopTrackAll()
                // cascades down through gbStopPattern()/gbStopNoteChannel(),
                // which also calls md_trackerVoiceStop() on every active
                // tracker voice.
                gbStopTrackAll();
                md_stopTone();
                currentGameIndex = -1;
    ...
    else if( currentGameIndex != -1 && justStarted )
    {
        ...
        // player actually confirms - the current game's own update() (and
        // therefore gbUpdateSoundTracker()) stops running entirely the
        // moment confirmingQuit is true, so nothing will restart this
        // while the dialog is up. If the player cancels instead, onResume()
        // (if the game defines one) or that game's own per-tick "restart
        // background music if it's not still playing" logic (several
        // already-shipped games have this, e.g. 101Starships) naturally
        // picks it back up once real gameplay updates resume - this isn't
        // a pause/resume mechanism, just an honest stop.
        gbStopTrackAll();
        md_stopTone();
        drawConfirmQuitDialog();
    }
